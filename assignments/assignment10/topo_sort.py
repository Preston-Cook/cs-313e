...
class Graph:
    ...
    def has_cycle(self):
        ...
        for i, _ in enumerate(self.vertices):
            if not self.vertices[i].was_visited() and self.dfs(i):
                return True
        return False
    ...
    def bfs(self, v):
        ...
        while not the_queue.is_empty():
            # get an adjacent unvisited vertex
            u = self.get_adj_unvisited_vertex(the_queue.dequeue())
            if u == -1:
                u = the_queue.dequeue()
            else:
                (self.vertices[u]).visited = True
    ...
    def toposort(self):
        ...
        adj_mat_copy = [row[:] for row in self.adj_mat]

        while vertices_copy:
            no_incoming_edges = [v for v in vertices_copy if all(
                row[self.get_index2(v.get_label(), vertices_copy)] == 0 for row in adj_mat_copy)]

            if not no_incoming_edges:
                raise ValueError("The graph has a cycle.")

            vertex = no_incoming_edges.pop()
            topo_list.append(vertex.get_label())
            self.delete_vertex2(vertex.get_label(),
                                adj_mat_copy, vertices_copy)
